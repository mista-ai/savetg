...
function getTrueMediaElement(x, y) {
    ...
    let target = elements.find(el => isTargetValid(el)) || null;
...
function populateMenuWithBotActions(bot_dict, menu, target) {
    Object.entries(bot_dict).forEach(([chatTitle, chatId]) => {
        ...
        button.onclick = function() {
            mediaUrl = getImageUrlFromTarget(target);
 ...
 */
function sendMediaWithText(mediaUrl, chatId, msgText) {
    ...
    chrome.storage.sync.get(['save_history', 'teleport_history'], (data) => {
        ...
        let teleport_history = data.teleport_history || {};
        ...
        if (!shouldSave || !(raw_link in teleport_history && chatId in teleport_history[raw_link])) {
            ...
            .then(result => {
                ...
                chrome.storage.sync.set({ teleport_history }, () => {
                    console.log('🌐 teleport_history updated in Chrome Sync:', teleport_history);
                });
            })
